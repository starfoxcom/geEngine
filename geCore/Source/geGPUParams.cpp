...
namespace geEngineSDK {
  ...
  bool GPUParamsBase::hasSamplerState(GPU_PROGRAM_TYPE::E type, const String& name) const
  {
    const SPtr<GPUParamDesc>& paramDesc = m_paramInfo->getParamDesc(type);
  ...
  bool GPUParamsBase::hasParamBlock(GPU_PROGRAM_TYPE::E type, const String& name) const
  {
    const SPtr<GPUParamDesc>& paramDesc = m_paramInfo->getParamDesc(type);
    if (paramDesc == nullptr)
      return false;
  ...
  GPUParamDataDesc* GPUParamsBase::getParamDesc(GPU_PROGRAM_TYPE::E type, const String& name) const
  {
    const SPtr<GPUParamDesc>& paramDesc = m_paramInfo->getParamDesc(type);
    if (paramDesc == nullptr)
      return nullptr;
  ...
  GPUParamBlockDesc* GPUParamsBase::getParamBlockDesc(GPU_PROGRAM_TYPE::E type, const String& name) const
  {
    const SPtr<GPUParamDesc>& paramDesc = m_paramInfo->getParamDesc(type);
    if (paramDesc == nullptr)
      return nullptr;
    ...
    : GPUParamsBase(paramInfo)
  {
    ...
    uint8* data = (uint8*)ge_alloc(totalSize);
    m_paramBlockBuffers = (ParamsBufferType*)data;
    for (uint32 i = 0; i < numParamBlocks; i++)
      new (&m_paramBlockBuffers[i]) ParamsBufferType();

    data += paramBlocksSize;
    m_sampledTextureData = (TextureData*)data;
    ...
    data += texturesSize;
    m_loadStoreTextureData = (TextureData*)data;
    ...
    m_buffers = (BufferType*)data;
    for (uint32 i = 0; i < numBuffers; i++)
      new (&m_buffers[i]) BufferType();

    data += buffersSize;
    m_samplerStates = (SamplerType*)data;
  ...
  template<bool Core>
  void TGPUParams<Core>::setParamBlockBuffer(GPU_PROGRAM_TYPE::E type, const String& name, const ParamsBufferType& paramBlockBuffer)
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
  ...
  void TGPUParams<Core>::setParamBlockBuffer(const String& name, const ParamsBufferType& paramBlockBuffer)
  {
    ...
    {
      const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc((GPU_PROGRAM_TYPE::E)i);
      if (paramDescs == nullptr)
        continue;
  ...
  template<bool Core>
  template<class T>
  void TGPUParams<Core>::getParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUDataParam<T, Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
  ...
  template<bool Core>
  void TGPUParams<Core>::getStructParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUParamStruct<Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
    ...
    auto iterFind = paramDescs->params.find(name);
    if (iterFind == paramDescs->params.end() || iterFind->second.type != GPU_PARAM_DATA_TYPE::kSTRUCT)
    {
      output = TGPUParamStruct<Core>(nullptr, nullptr);
      LOGWRN("Cannot find struct parameter with the name '" + name + "'");
    }
  ...
  template<bool Core>
  void TGPUParams<Core>::getTextureParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUParamTexture<Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
  ...
  template<bool Core>
  void TGPUParams<Core>::getLoadStoreTextureParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUParamLoadStoreTexture<Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
  ...
  template<bool Core>
  void TGPUParams<Core>::getBufferParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUParamBuffer<Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
  ...
  template<bool Core>
  void TGPUParams<Core>::getSamplerStateParam(GPU_PROGRAM_TYPE::E type, const String& name, TGPUParamSampState<Core>& output) const
  {
    const SPtr<GPUParamDesc>& paramDescs = m_paramInfo->getParamDesc(type);
    if (paramDescs == nullptr)
    {
  ...
  {
    uint32 globalSlot = m_paramInfo->getSequentialSlot(PARAM_TYPE::kLoadStoreTexture, set, slot);
    if (globalSlot == (uint32)-1)
      return TGPUParams<Core>::TextureType();
  ...
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<float>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<float, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<int>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<int, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<LinearColor>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<LinearColor, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector2>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector2, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector3>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector3, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector4>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector4, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector2I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector2I, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector3I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector3I, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Vector4I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector4I, false>&) const;
  template GE_CORE_EXPORT void TGPUParams<false>::getParam<Matrix4>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Matrix4, false>&) const;
  
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<float>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<float, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<int>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<int, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<LinearColor>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<LinearColor, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector2>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector2, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector3>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector3, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector4>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector4, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector2I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector2I, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector3I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector3I, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Vector4I>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Vector4I, true>&) const;
  template GE_CORE_EXPORT void TGPUParams<true>::getParam<Matrix4>(GPU_PROGRAM_TYPE::E type, const String&, TGPUDataParam<Matrix4, true>&) const;
  ...
  SPtr<geCoreThread::GPUParams> GPUParams::getCore() const
  {
    return std::static_pointer_cast<geCoreThread::GPUParams>(m_coreSpecific);
  }

  SPtr<geCoreThread::CoreObject> GPUParams::createCore() const
  {
    SPtr<GPUPipelineParamInfo> paramInfo = std::static_pointer_cast<GPUPipelineParamInfo>(m_paramInfo);

    return geCoreThread::HardwareBufferManager::instance().createGPUParams(paramInfo->getCore());
  }
  ...
  CoreSyncData GPUParams::syncToCore(FrameAlloc* allocator)
  {
    ...
    uint32 loadStoreTextureArraySize = numStorageTextures * sizeof(SPtr<geCoreThread::Texture>);
    ...
    SPtr<geCoreThread::GPUParamBlockBuffer>* paramBuffers = (SPtr<geCoreThread::GPUParamBlockBuffer>*)(data + paramBufferOffset);
    SPtr<geCoreThread::Texture>* textures = (SPtr<geCoreThread::Texture>*)(data + textureArrayOffset);
    SPtr<geCoreThread::Texture>* loadStoreTextures = (SPtr<geCoreThread::Texture>*)(data + loadStoreTextureArrayOffset);
    SPtr<geCoreThread::GPUBuffer>* buffers = (SPtr<geCoreThread::GPUBuffer>*)(data + bufferArrayOffset);
    SPtr<geCoreThread::SamplerState>* samplers = (SPtr<geCoreThread::SamplerState>*)(data + samplerArrayOffset);
