...
namespace geEngineSDK {
  class GE_UTILITY_EXPORT BinaryCloner
  {
   public:
     ...
     *            as well, otherwise the references to the original objects
     *            will be kept.
     */
    static SPtr<IReflectable>
    clone(IReflectable* object, bool shallow = false);

   private:
    struct ObjectReferenceData;

    /**
     * @brief Identifier representing a single field or an array entry in an
     *        object.
     */
    struct FieldId
    {
    ...
    /**
     * @brief A saved reference to an object with a field identifier that owns
     *        it.
     */
    ...
    /**
     * @brief Contains all object references in a portion of an object belonging
     *        to a specific class (base and derived classes count as separate
     *        sub-objects).
     */
    ...
    /**
     * @brief Contains all object references in an entire object, as well as
     *        the identifier of the field owning this object.
     */
    struct ObjectReferenceData
    {
      FieldId fieldId;
    ...
    /**
     * @brief Iterates over the provided object hierarchy and retrieves all
     *        object references which are returned in @p referenceData output
     ...
     *        applies them to a specific object. Type of the object must be the
     *        same as the type that was used when calling gatherReferences().
     */
    static void
